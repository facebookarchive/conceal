...
foreach (@ARGV, split(/ /, $options))
	...
	elsif (/^no-chacha-poly$/){ $no_chacha_poly=1; }
	}
...
$crypto.=" crypto/chacha20_poly1305/chacha20poly1305.h" unless $no_chacha_poly;
...
sub is_valid
{
	...
	sub recognise
	{
		...
		if ($platforms) {
			...
			if ($keyword eq "CHACHA_POLY" && $no_chacha_poly) { return 0; }
